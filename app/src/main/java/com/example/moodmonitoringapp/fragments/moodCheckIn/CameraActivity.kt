...
import com.example.moodmonitoringapp.databinding.ActivityCameraBinding
import com.example.moodmonitoringapp.databinding.FragmentEditProfileBinding
import com.example.moodmonitoringapp.fragments.moodCheckIn.EmotionPyTorchVideoClassifier.Companion.applyToUiAnalyzeImageResult
import com.example.moodmonitoringapp.mtcnn.Box
...
class  CameraActivity : BaseModuleActivity() {
    ...
    private fun getContentViewLayoutId(): Int {
        return com.example.moodmonitoringapp.R.layout.activity_camera
    }

    private fun getCameraPreviewTextureView(): TextureView {
        return findViewById(com.example.moodmonitoringapp.R.id.object_detection_texture_view)
    }
        ...
        imageAnalysis.analyzer =
            ImageAnalysis.Analyzer { image: ImageProxy?, rotationDegrees: Int ->
                if (SystemClock.elapsedRealtime() - mLastAnalysisResultTime > 500) {
                    runOnUiThread {
                        val overlay: ImageView = findViewById(com.example.moodmonitoringapp.R.id.overlay)
                        overlay.setImageResource(android.R.color.transparent)
                        val text: TextView = findViewById(com.example.moodmonitoringapp.R.id.liveText)
                ...
                if (result != null) {
                    ...
                    runOnUiThread {
                        ...
                        applyToUiAnalyzeImageResult(result, height, width, findViewById(com.example.moodmonitoringapp.R.id.overlay))}
                }
            }
